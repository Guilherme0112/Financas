...
import HeaderSecao from "../Dashboard/Components/HeaderSecao.vue";
...
const mesesAtivos = computed(() => {
    ...
    return props.media.filter(
        (item) => Number(item.entradas) > 0 || Number(item.saidas) > 0,
    );
});
...
const mediaEntrada = computed(() => {
    ...
    const soma = mesesAtivos.value.reduce(
        (acc, item) => acc + Number(item.entradas),
        0,
    );
...
const mediaSaida = computed(() => {
    ...
    const soma = mesesAtivos.value.reduce(
        (acc, item) => acc + Number(item.saidas),
        0,
    );
    return soma / totalMeses;
});
...
const optimizedSurplus = computed(() => {
    const despesaOtimizada =
        mediaSaida.value * (1 - optimizationLevel.value / 100);
    ...
    <AuthenticatedLayout>
        <div class="py-6 space-y-4">
            <div class="p-6 bg-white rounded-2xl min-h-screen font-sans shadow-lg">
                <HeaderSecao
                    :icon="TrendingUp"
                    title="Prospecção de Futuro"
                    description="Simule cenários e visualize a saúde do seu patrimônio nos próximos meses"
                    icon-color="text-rose-800"
                />
                <div class="grid grid-cols-1 lg:grid-cols-4 gap-4 mb-8 mt-4">
                    ...
                    <ProjectedBalanceCard
                        :balance="
                            monthlyProjection.length > 0
                                ? monthlyProjection[monthlyProjection.length - 1]
                                    .total
                                : 0
                        "
                    />
