    ...
    <table class="w-full text-sm">
      ...
      <thead class="border-b" :class="themeClasses.thead">
        <tr>
          ...
          <th v-if="actions" class="px-6 py-3 text-center text-xs font-bold uppercase tracking-wider"
      ...
      <tbody>
        <tr v-if="rows.length === 0">
          <td :colspan="headers.length + (actions ? 1 : 0)" :class="themeClasses.tbody + 'px-3 py-3 text-center text-sm text-zinc-500'">
            Nenhum registro encontrado
          </td>
        </tr>

        <tr v-else v-for="(row, index) in rows" :key="index" class="border-b last:border-0 transition"
          ...
          <!-- AÇÕES -->
          <td v-if="actions" class="px-6 py-3 text-center relative" data-menu>
            <button @click="toggle(index)" class="p-2 rounded-full transition rotate-90"
            ...
            <div v-if="aberto === index" class="absolute right-4 mt-2 w-44 bg-white rounded-md shadow-xl z-50 border"
